...
class Formulario extends Component {
    ...
    adicionar() {
        if (this.nome.current.value === "") {
            alert("Digite o nome do produto");
            this.nome.current.value = "";
        } else if (this.preco.current.value === "" || isNaN(this.preco.current.value)) {
            alert("Digite o preço");
            this.preco.current.value = "";
        } else if (isNaN(this.quantidade.current.value) || this.quantidade.current.value === "" ||
            this.quantidade.current.value.indexOf(".") >= 0) {
